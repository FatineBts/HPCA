...
#pragma omp parallel private(d, x, prev) reduction(+: r,b,t) shared(j, absorbed)
{
 init_uniform_random_number(omp_get_thread_num());

#pragma omp for schedule(static,paquetN)
  for (i = 0; i < n; i++) { //l'indice i est privé par défaut 
	...
	b++;
        prev = j; //prev est propre à chaque thread donc doit être privé  
	#pragma omp atomic 
	j++; 
        absorbed[prev] = x;
	break;
